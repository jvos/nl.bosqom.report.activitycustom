...
/**
 * BOSW1604294 insite - activiteiten rapporten
 * Request to add the fields Persoonsnummer First, VGE nummer and Details
 * 
 * This is a copy of the CRM_Report_Form_Activity with a couple adjustment.
 */
class CRM_Activitycustom_Form_Report_ActivityCustom extends CRM_Report_Form {
  ...
  function __construct() {
     ...
     * Request to add the fields Persoonsnummer First, VGE nummer and Details
     */
    $configActivitycustom = CRM_Activitycustom_Config::singleton();
    
    $this->_columns['aanvullende_persoonsgegevens'] = array(
      'dao' => 'CRM_Contact_DAO_Contact',
      'fields' => array(),
      'grouping' => 'aanvullende_persoonsgegevens',
      'group_title' => 'Aanvullende Persoonsgegevens',
    );
    
    foreach($configActivitycustom->perGegevensCustomFields as $name => $custom_field){
      $this->_columns['aanvullende_persoonsgegevens']['fields'][$name] = array(
          'name' => $name,
          'title' => $custom_field['label'],
      );
    }
      
    $this->_columns['huurovereenkomst_huishouden'] = array(
      'dao' => 'CRM_Contact_DAO_Contact',
      'fields' => array(),
      'grouping' => 'huurovereenkomst_huishouden',
      'group_title' => 'Huurovereenkomst (huishouden)',
    );
    
    foreach($configActivitycustom->hovHouseholdCustomFields as $name => $custom_field){
      $this->_columns['huurovereenkomst_huishouden']['fields'][$name] = array(
          'name' => $name,
          'title' => $custom_field['label'],
      );
    }
      ...
      if (array_key_exists('fields', $table)) {
        foreach ($table['fields'] as $fieldName => $field) {
          if (CRM_Utils_Array::value('required', $field) ||
            ...
            if('aanvullende_persoonsgegevens' != $tableName and 'huurovereenkomst_huishouden' != $tableName){
  ...
  function alterDisplay(&$rows) {
    ...
    foreach ($rows as $rowNum => $row) {
       ...
       * Request to add the fields Persoonsnummer First, VGE nummer and Details
       */
      $hoofdhuurder_id = $configActivitycustom->getHoofdhuurder($row['civicrm_activity_target_target_contact_id']);
      $household_id = $configActivitycustom->getHousehold($row['civicrm_activity_target_target_contact_id']);
      
      $per = [];
      if($hoofdhuurder_id){
        $per = $configActivitycustom->getPerNummerFirst($hoofdhuurder_id);
      }
      
      $hov = [];
      if($household_id){
        $hov = $configActivitycustom->getHovHousehold($household_id);
      }
      
      foreach($this->_columns['aanvullende_persoonsgegevens']['fields'] as $fieldName => $field){
        if (CRM_Utils_Array::value('required', $field) || CRM_Utils_Array::value($fieldName, $this->_params['fields'])) {
          if(!empty($per)){
            $rows[$rowNum]['aanvullende_persoonsgegevens_' . $fieldName] = $per[$fieldName];
          }
        }
      }
      
      foreach($this->_columns['huurovereenkomst_huishouden']['fields'] as $fieldName => $field){
        if (CRM_Utils_Array::value('required', $field) || CRM_Utils_Array::value($fieldName, $this->_params['fields'])) {
          if(!empty($hov)){
            $rows[$rowNum]['huurovereenkomst_huishouden_' . $fieldName] = $hov[$fieldName];
          }
        }
      }
